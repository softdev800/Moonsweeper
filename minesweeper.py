...
try:
    # Include in try / except block if you are targeting Mac / Linux too
...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self._timer.start(1_000)  # 1 second timer
    ...
    def init_map(self):
        ...
        for x in range(0, self.b_size):
            for y in range(0, self.b_size):
                ...
                w.clicked.connect(self.trigger_start)  # Connect signal to handle expansion.
                w.expandable.connect(self.expand_reveal)  # Connect signal to handle expansion.
                w.ohno.connect(self.game_over)  # Connect signal to handle expansion.
    ...
    def reset_map(self):
        ...
        while len(positions) < self.n_mines:
            x, y = randint(0, self.b_size - 1), randint(0, self.b_size - 1)
    ...
    def trigger_start(self, *args):
        if self.status != STATUS_PLAYING:
            self.update_status(STATUS_PLAYING)  # First click.
            self._timer_start_nsecs = int(time.time())  # Start timer.
